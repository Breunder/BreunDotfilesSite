...
// Deze functie controleert of een URL bestaat
...
// Deze functie wordt uitgevoerd wanneer op een link wordt geklikt
function handleLinkClick(event) {
    const link = event.currentTarget;
    const href = link.getAttribute('href');
    
    // Negeer externe links, ankers, of javascript: links
    if (!href || 
    ...
    // Controleer of de URL bestaat
    checkIfUrlExists(href, function(exists) {
        if (exists) {
            // Als de URL bestaat, ga er naartoe
            window.location.href = href;
        } else {
            // Als de URL niet bestaat, sla de URL op en ga naar de 404-pagina
            localStorage.setItem('lastRequestedUrl', href);
